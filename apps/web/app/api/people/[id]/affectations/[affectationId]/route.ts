...
/// Proxy serveur vers NestJS — changement de statut d'une affectation
/// (Actif / Inactif / Suspendu), réservé au RH de la société concernée.
export async function PATCH(request: NextRequest, { params }: { params: { id: string; affectationId: string } }) {
  ...
  try {
    const result = await apiFetch(`/people/${params.id}/affectations/${params.affectationId}`, {
      method: "PATCH",
